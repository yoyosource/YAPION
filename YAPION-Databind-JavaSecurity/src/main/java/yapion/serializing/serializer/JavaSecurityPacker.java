...
public class JavaSecurityPacker {

    public static void main(String[] args) throws Exception {
        Class<?> clazz = Class.forName(Thread.currentThread().getStackTrace()[1].getClassName());

        String s = clazz.getProtectionDomain().getCodeSource().getLocation().getFile();
        s += clazz.getPackage().getName().replace('.', '/');

        File source = new File(s);
        if (!source.exists()) return;

        File destination = new File(s.substring(0, s.lastIndexOf('/')), "java-security.pack");
